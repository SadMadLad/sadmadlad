...
export default function Section({ title, entries }) {
  ...
  return (
    <section className="px-2 md:px-0">
      ...
      <AnimatePresence>
        {isExpanded &&
          entries.map((entry, index) => <DiaryEntry key={index} title={entry.title} type={entry.type} updatedOn={entry.updated_on} description={entry.description} solution={entry.solution} />)
        }
      </AnimatePresence>
    </section>
  )
